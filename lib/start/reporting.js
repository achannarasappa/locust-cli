...
const termQueue = require('../term/queue');
const termSummary = require('../term/summary');
const term = require('../term');

const renderSummary = termSummary.render;
const renderTerminal = term.render;
const startRefreshingQueueInTerminal = termQueue.refreshQueue;
...
const _executeWithProgressReporting = async (
  redisDbResult,
  redisDbQueue,
  jobDefinition,
  renderer,
...
const _pollQueueWhileActive = (redisDbQueue, jobDefinition) => (
...
const start = async (redisDbQueue, redisDbResult, jobDefinition, onTerminalExit) => {
  ...
  const terminal = renderTerminal(async () => {

    clearInterval(queueRefreshInterval);
    await onTerminalExit();

  });
  const renderer = renderSummary(terminal);
